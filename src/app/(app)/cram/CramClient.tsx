 ...
 */
export default function CramClient() {
  // `?topic=` arrives from a topic's Learn row, so "practise this topic" lands
  // here with it already ticked rather than making the reader find it again in
  // the list.
  const preselected = useSearchParams().get('topic');
  const [topics, setTopics] = useState<CramTopic[] | null>(null);
  const [picked, setPicked] = useState<Set<string>>(new Set(preselected ? [preselected] : []));
  ...
  // know about.
  useEffect(() => {
    if (!preselected || !topics) return;
    if (!topics.some((t) => t.id === preselected && t.cardCount > 0)) {
      setPicked((p) => {
        const next = new Set(p);
        next.delete(preselected);
        return next;
      });
    }
  }, [preselected, topics]);
            ...
            <div className="card">
              ...
              {result && (
                <div className={`mt-5 rounded-[11px] px-4 py-3.5 ${result.verdict.correct ? 'bg-good/10' : 'bg-destructive/10'}`}>
                  ...
                  {/* One place decides how the right answer is shown, for both
                      kinds: nonsense answers prove nothing to the reader. */}
                  {(result.modelAnswer || result.primaryAnswer) && (
                    <div className="inset mt-2.5 px-3 py-2">
                      <span className="t-micro uppercase tracking-[0.08em] text-muted-foreground">
                        {result.modelAnswer ? 'Model answer' : 'Answer'}
                      </span>
                      <p className="t-caption mt-0.5 text-foreground">{result.modelAnswer ?? result.primaryAnswer}</p>
                    </div>
                  )}
